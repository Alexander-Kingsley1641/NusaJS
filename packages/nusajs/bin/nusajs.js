...
async function run() {
  ...
  if (command === "dev") {
    await import(
      pathToFileURL(path.join(frameworkRoot, "src/server/dev.js")).href
    );
    return;
  }
  ...
  if (command === "build") {
    await import(
      pathToFileURL(path.join(frameworkRoot, "src/server/build.js")).href
    );
    return;
  }
  ...
  if (command === "preview") {
    await import(
      pathToFileURL(path.join(frameworkRoot, "src/server/preview.js")).href
    );
    return;
  }

  if (command === "bot") {
    const mod = await import("nusajs-discord");
