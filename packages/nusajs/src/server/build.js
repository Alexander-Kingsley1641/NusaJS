...
async function build() {
  ...
  const runtimeCandidates = [
    path.resolve(root, "node_modules", "nusajs-dom", "src"),
    path.resolve(frameworkRoot, "..", "nusajs-dom", "src"),
  ];
  const runtimeSource = runtimeCandidates.find((candidate) =>
    fsSync.existsSync(candidate)
  );
  const runtimeDest = path.join(outDir, "@nusajs", "runtime", "web");
  ...
  if (runtimeSource) {
    ...
    logger.warn("Runtime web tidak ditemukan. Build tetap berjalan.");
  }
